...
theorem siegelsLemma (hn: m < n) (hm: 0 < m) (hA : A ≠ 0 ) : ∃ (t: Fin n → ℤ), t ≠ 0 ∧
   ...
   have hePos : 0 < e := div_pos (cast_pos.mpr hm)  (sub_pos_of_lt (cast_lt.mpr hn))
   ...
   have hone_le_n_a : 1 ≤ n * a := one_le_mul (one_le_of_lt hn) ha.2

   have hcardineq : S.card<T.card := by
      zify
      rw [hcardT, hcardS]
      calc
      ∏ i : Fin m, (P i - N i + 1) ≤ C^m := by   --recall C:=n*a*B+1
            rw [<-Fin.prod_const m (C : ℤ)]
            apply Finset.prod_le_prod  --2 goals
            all_goals intro i _
            linarith [hineq2 i] --first goal done
   ...
   --Pigeonhole
   rcases Finset.exists_ne_map_eq_of_card_lt_of_maps_to hcardineq him
