...
private lemma card_S_lt_card_T : (S).card < (T).card := by
  ...
  ∏ i : Fin m, (P i - N i + 1) ≤ (n * a * B + 1) ^ m := by
      ...
      _  ≤ (n * a) ^ m * (B + 1) ^ m := by
        ...
        norm_cast
        apply one_le_mul (one_le_of_lt hn)
        sorry
      _ < (B + 1) ^ (n - m) * (B + 1) ^ m := by
...
theorem exists_ne_zero_int_vec_norm_le  (hA_nezero : A ≠ 0)  : ∃ (t : Fin n → ℤ), t ≠ 0 ∧
  ...
  rcases Finset.exists_ne_map_eq_of_card_lt_of_maps_to (card_S_lt_card_T m n A hn) (image_T_subset_S m n A) with ⟨ x, hxT,y, hyT ,hneq, hfeq⟩
  ...
  ---Inequality
  have n_mul_norm_A_pow_e_nonneg : 0 ≤ (n * ‖A‖)^e := by sorry
    /- rw [norm_eq_natNorm]
